...
argument-hint: Describe the symptom, expected vs actual behavior, reproduction steps, and any relevant frontend error output
tools: [search, read, edit, execute, todo, agent, 'vscode/askQuestions', 'vscode/memory']
agents: ['Explore']
---
...
**Current debug note**: `/memories/session/frontend-debug.md` - update using #tool:vscode/memory only when an investigation spans multiple turns.
...
<constraints>
- Use #tool:vscode/askQuestions early when reproduction steps, expected behavior, screenshots, or error text are missing.
...
- If needed, use #tool:vscode/askQuestions to collect actual vs expected behavior, reproduction steps, recent regression context, environment or build variant, and any relevant logs.
- Capture a short session note in `/memories/session/frontend-debug.md` only for multi-turn investigations.
...
- Use the `Explore` subagent only when a read-only parallel search will materially speed up discovery.
