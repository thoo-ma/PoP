-- ============================================================================
-- Seed POOP balance for dev mode users
--
-- Extends seed_dev_test_nfts to also reset the user's poop_balance to a
-- generous dev amount so testers can exercise breed / repair flows immediately.
-- ============================================================================

CREATE OR REPLACE FUNCTION public.seed_dev_test_nfts(p_user_id UUID)
RETURNS JSON AS $$
DECLARE
  v_supabase_url  TEXT := 'https://mtnluwkvhkwwxvxdtkgs.supabase.co';
  v_box_base      TEXT;
  v_dev_poop      INTEGER := 10000;
BEGIN
  ...
  -- ── POOP balance (dev seed) ───────────────────────────────────────────────
  UPDATE public.users
     SET poop_balance = v_dev_poop
   WHERE id = p_user_id;
  ...
  RETURN json_build_object(
    ...
    'nfts',         50,
    'mystery_boxes', 10,
    'poop_balance', v_dev_poop
  );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

COMMENT ON FUNCTION public.seed_dev_test_nfts(UUID) IS
  'Seeds 50 test NFTs + 10 mystery boxes + 10 000 POOP for the given user. '
