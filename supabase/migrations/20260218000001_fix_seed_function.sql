...
BEGIN
  ...
  -- For dev mode: always ensure 50 NFTs by clearing existing ones first
  -- This allows the function to be called multiple times and always get a fresh set
...
COMMENT ON FUNCTION public.seed_dev_test_nfts() IS 'Seeds 50 test NFTs for current user (dev/testing only). Deletes existing NFTs first to ensure a fresh set of exactly 50.';
