...
const DIFF_COLORS: Record<string, string> = {
  easy: '#22c55e', normal: '#f59e0b', strict: '#ef4444',
}
  ...
  const handlePreset = (difficulty: string, field: PresetField, value: string) => {
    ...
    setDraft('sensors', {
      SENSOR_PRESETS: {
        ...
        [difficulty]: { ...sensors.SENSOR_PRESETS[difficulty as typeof DIFFICULTIES[number]], [field]: num },
      },
    })
  }
    ...
    <div className="space-y-6">
      ...
      <Tabs defaultValue="presets" className="w-full">
        ...
        <TabsContent value="presets" className="space-y-4 mt-4">
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-neutral-800 text-[11px] uppercase tracking-wider text-neutral-500">
                    ...
                    {DIFFICULTIES.map((d) => (
                      <th key={d} className="px-3 py-2 text-center capitalize font-medium" style={{ color: DIFF_COLORS[d] }}>
                        {d}
                      </th>
                    ))}
                  </tr>
          ...
          <Card className="border-neutral-800 bg-neutral-900/50">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-neutral-300">Difficulty Profile (Radar)</CardTitle>
            </CardHeader>
            <CardContent>
        ...
        <TabsContent value="audio" className="space-y-4 mt-4">
              ...
              <div className="grid grid-cols-3 gap-4">
                {DIFFICULTIES.map((d) => (
                  <div key={d} className="space-y-1.5">
                    <Label className="text-xs font-medium capitalize" style={{ color: DIFF_COLORS[d] }}>
                      {d}
                    </Label>
                    <NumberInput
                      step={0.05}
          ...
          <Card className="border-neutral-800 bg-neutral-900/50">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-medium text-neutral-300">Threshold by Difficulty</CardTitle>
            </CardHeader>
            <CardContent>
