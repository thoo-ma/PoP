...
export default function LootRollPanel() {
  const loot = useGameConfigStore(
    useShallow((s) => ({ ...s.config.loot_roll, ...s.drafts.loot_roll })),
  )
      ...
      <Card className="border-neutral-800 bg-neutral-900/50">
        ...
        <CardContent>
          <code className="text-sm text-neutral-400">
            P(win) = min(1, <span className="text-blue-400">{loot.BASE_WIN_PROBABILITY}</span> +
            holds × <span className="text-green-400">{loot.PER_HOLD_INCREMENT}</span>)
          </code>
